...
my $h=0;
my $tag_multi=0;
GetOptions ('help' => \$h, 'h' => \$h, 'm'=>\$tag_multi);
if ($h==1 || $ARGV[0] eq ""){ # If asked for help or did not set up any argument
	print "# Script to calculate how many positions are covered by spacers for each UViG
# Arguments :
# run
# -m: to run on multitaxa viruses
";
	die "\n";
}
...
my $out_file="uvig_hq_coverage_by_spacer.tsv";
my $log_file="uvig_hq_coverage_list_pblm.txt";


if ($tag_multi==1){
    $cl_to_array="All_spacers_vr_additional_multitaxa.tsv";
    $hit_file="All_hits_vr_additional_multitaxa.tsv";
    $out_file="uvigadditional_multitaxa_coverage_by_spacer.tsv";
    $log_file="uvig_additional_multitaxa_coverage_list_pblm.txt";
}
...
open my $s1,">",$out_file;
print $s1 "uvig\tarray\tlength\tcovered\tpercent\tn_unique_spacer\tmedian_hit_per_spacer\tmax_hit\n";
...
while(<$tsv>){
    ...
    if (defined($store_cover{$tab[0]})){
        foreach my $array (keys %{$store_cover{$tab[0]}}){
            ...
            if ($covered > 0){
                print $tab[0]."\t".$array."\t".$tab[3]."\t".$covered."\t".$ratio."\t".$n_unique."\t".$median_hit."\t".$max_hit."\n";
                print $s1 $tab[0]."\t".$array."\t".$tab[3]."\t".$covered."\t".$ratio."\t".$n_unique."\t".$median_hit."\t".$max_hit."\n";
            }
        }
    }
}
