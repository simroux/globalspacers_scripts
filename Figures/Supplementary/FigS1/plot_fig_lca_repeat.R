...
## Panel B
## New plot in revision: how many repeats have a predicted type, how many have a connected Cas gene, how many have corresponding type across both
source("Cas_gene_overview_sourcedata.R")
summary(cas_type_source)
fit1 <- euler(cas_type_source)
plot(fit1,legend = list (labels=c("all repeat clusters","type predicted from repeat sequence","type predicted from neighboring Cas loci")))
pdf("Supp_mat/Fig_S1_Cas_prediction.pdf",width=5,height=3)
plot(fit1,legend = list (labels=c("all repeat clusters","type predicted from repeat sequence","type predicted from neighboring Cas loci")))
dev.off()